...
describe('Screenplay: Guards available/variants', function () {
  this.timeout(90000);
  let driver;
  let user;
  ...
  it('/availableVehicles -> /signin', async () => {
    await user.attemptsTo(Open.url(`${BASE}/availableVehicles`));
    const url = await user.asks(CurrentUrl);
    expect(url).to.include('/signin');
  });

  it('/allVariants -> /signin', async () => {
    await user.attemptsTo(Open.url(`${BASE}/allVariants`));
    const url = await user.asks(CurrentUrl);
    expect(url).to.include('/signin');
  });
});
